...
namespace HotKeySetter
{
    public class BezierCurve
    {
        ...
        /// <param name="step">步长,步长越小，轨迹点越密集。如0.01为100个点。</param>
        /// <returns></returns>
        public static PointF[] Draw(PointF[] points, int count, float step)
        {
            List<PointF> bezier_curves_points = new List<PointF>();
            float t = 0F;
            do
            {
                PointF temp_point = GetInterpolation(t, points, count);    // 计算插值点
                t += step;
        ...
        /// <param name="k"></param>
        /// <returns></returns>
        private static ulong CalcCombinationNum(int n, int k)
        {
        ...
        /// <returns></returns>
        public static PointF GetInterpolation(float t, PointF[] points, int count)
        {
