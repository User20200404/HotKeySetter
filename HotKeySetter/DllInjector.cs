...
namespace HotKeySetter
{
    ...
    {
        /// <summary>
        /// 为指定进程注入OpenProcess拦截器。
        /// </summary>
        /// <param name="pid">进程的ID，若为0，则向所有进程注入DLL。</param>
        /// <param name="protectedID">要保护的进程ID。</param>
        public static void HookOpenProcess(int pid,int protectedID)
        {
            ConfigFile.WriteStatic(ConfigFile.HotKeySetterConfigDirectory + @"\Hook.ini", "Ring3Hook", "protectedID", protectedID.ToString());
            string[] args = new string[2];
            args[0] = pid.ToString();
            args[1] = System.Environment.CurrentDirectory + @"\DllHookProc.dll";
            DllInjector.InjectMain(args);
        }
    }
